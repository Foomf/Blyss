...
void tile_grid::set_tile(std::int32_t x, std::int32_t y, const std::string& file_path, std::int32_t tx, std::int32_t ty)
{
    const auto index = get_index(x, y);
    sprites_[index] = std::make_unique<sprite>(shader_, texture_cache_, file_path, tx, ty);
}
